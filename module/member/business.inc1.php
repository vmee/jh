...
switch($step){
	case '1':
		...
		if($post){
			...
			$head_title .= '第一步';
			include template('business_reg1', $module);
			break;
		}
	case '2':
		if($action != crypt_action('register')) dalert($L['check_sign'].'(1)');
		if(!$submit && !$post) dheader('business.php');
		...
		set_cookie('business_post', json_encode($post));

		$head_title .= '第二步';
		include template('business_reg2', $module);
		break;
	case '3':
		if($action != crypt_action('register')) dalert($L['check_sign'].'(1)');
		$cookie_post = get_cookie('business_post');
		...
		if($do->add($post)) {
			...
			$head_title .= '第三步';
			include template('business_reg3', $module);
		} else {
			$reload_captcha = $MOD['captcha_register'] ? reload_captcha() : '';
			$reload_question = $MOD['question_register'] ? reload_question() : '';
