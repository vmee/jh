...
if($submit) {
	...
	if(!is_mobile($username)) message('您输入的手机号不正确');
	if(strlen($active_code) != 6) message('您的邀请码不正确');
	$goto = isset($goto) ? true : false;
	if($goto) $forward = $MOD['linkurl'];

    $r = $db->get_one("SELECT itemid,password FROM {$DT_PRE}invite_customer WHERE regtime=0 and `mobile`='$username'");
    if($r){
        if($r['password'] == $active_code){
            message('欢迎注册本站会员，请先完善注册信息', linkurl('member/register.php?activeauth='.encrypt($r['itemid'].'|'.$username.'|'.$active_code)));
        }else{
            message('您的邀请码输入不正确');
        }

    }else{
        message('不好意思，没有查询到您的被邀请记录; 请联系本地合作商家或联系本站客服.');
    }
	...
	if($r) {
		...
		if($option == 'username' || $option == 'passport') {
            ...
            echo $password;
            //检查是不是受邀请用户
            if(strlen($password) == 6 && ($option == "mobile" || $option == "email")){
                $r = $db->get_one("SELECT itemid FROM {$DT_PRE}invite_customer WHERE regtime=0 and `$option`='$username' and `passsword`='$password'");
                if($r){
                    message('欢迎注册本站会员，请先完善信息', linkurl('member/register.php'));
                }
	...
	include template('active_login', $module);
}
?>