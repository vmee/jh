...
function userinfo($username, $cache = 1) {
	...
	$user_company = $db->get_one("SELECT * FROM {$db->pre}company c WHERE username='$username'");
...
function invite_company_name($userid){

    global $db;
    $userid = intval($userid);
    $result = $db->get_one("SELECT userid FROM {$db->pre}invite_customer WHERE reg_userid=$userid", 'CACHE');

    $company_name = '';
    if($result){
        $invite_user_id = $result['userid'];
        $company_arr = $db->query("SELECT company FROM {$db->pre}company WHERE userid=$invite_user_id");

        $company_name = $company_arr['company'];
    }
